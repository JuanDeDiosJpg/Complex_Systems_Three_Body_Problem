...
int main()
{
    ...
    if (M0.is_open()==true&&M1.is_open()==true&&M2.is_open()==true&&Hamiltoniano.is_open()==true)
    {
        ...
        double h=50*60; //Eso es para casos más estables como la figure 8
        int i, j, k;//Contador para calcular nuevos valores de posiciones y tal

        //Para emplear sistemas que sabemos ya cómo interactúan en principio voy a emplear las dimensiones de la Tierra y la Luna. El tercer cuerpo será otra Luna. Podré ir cambiando a otros sistemas
...
double Calc_Prpunto (vector<double> r, vector<double> phi, vector<double> Pphi, vector<double> m, double h, int i)//El contador i es para saber en qué cuerpo estoy y aplicar un cálculo u otro
{
    ...
    int j = (i + 1) % 3; // Si i=0 -> j=1. Si i=1 -> j=2. Si i=2 -> j=0
    int k = (i + 2) % 3; // Si i=0 -> k=2. Si i=1 -> k=0. Si i=2 -> k=1
    ...
    // Protección centrífuga: si r[i] es cero, el término Pphi^2 / r^3 no se añade
